...
Агрегированные чеки из 1С используются для обучения item-to-item независимо
от межсистемной идентификации. MVP работает на уровнях персонализации `none`
и `session`; уровень `identified` и индивидуальные исключения офлайн-покупок
включаются только после подтверждения на Discovery связи сайта с клиентом 1С.
...
curl -H "X-Api-Key: local-dev" \
  "http://localhost:4010/get_recommendation?slot=product_card_cross_sell&product_id=SKU-100234"
```
...
- **Персонализация учитывает доступную идентичность:** внутренний Identity
  Resolver выбирает `none`, `session` или `identified`; офлайн-покупки
  исключаются только для подтверждённо связанного профиля.
